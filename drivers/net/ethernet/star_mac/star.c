...
#if  defined(CONFIG_OPM)||defined(CONFIG_ARCH_MT5881)||defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882)
...
{	
#if defined(CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882)
    u32 u4Reg;
    ...
    if(this_device!=NULL)
    {
        #if defined (CONFIG_ARCH_MT5398) || defined (CONFIG_ARCH_MT5399) || defined (CONFIG_ARCH_MT5880) || defined (CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882)
	...
	STAR_MSG(STAR_DBG, "star_suspend 0x32308 value(0x%x)\n", StarGetReg(eth_base+0x308));

        if(StarGetReg(eth_base+0x8)&(1<<30))
        {
           StarSetReg(eth_base+0x54,0x1ef);           
        }		
	STAR_MSG(STAR_DBG, "star_suspend 0x54 register is %x\n", StarGetReg(eth_base + 0x54));
        
	u4Reg =  StarGetReg(eth_base+0x300);
	u4Reg |= 1<<16;
	StarSetReg(eth_base+0x300,u4Reg);
	STAR_MSG(STAR_DBG, "star_suspend 0x300 register is %x\n", StarGetReg(eth_base + 0x300));
...
static  int star_resume(struct device *dev)
{
	...
	if(this_device!=NULL)
	{
        ...
        STAR_MSG(STAR_DBG, "star_resume 0x300 register is %x\n", StarGetReg(eth_base + 0x300));
        ...
        {
    	    if(star_open(this_device) !=0)
            {
                STAR_MSG(STAR_ERR, "%s,star_open fail!!!\n", __FUNCTION__);;
            }
        }
        #ifdef SUPPORT_ETH_POWERDOWN
        if(Ethernet_resume() != 0)
        {
